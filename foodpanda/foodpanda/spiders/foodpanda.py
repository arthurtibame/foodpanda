...
class FoodPandaSpider(scrapy.Spider):
    ...
    def parse(self, response):           
        ...
        poi_id = api1_data.get('id')
        #poi_name
        poi_name = api1_data.get('name')
        #poi_address
        poi_address =api1_data.get('address')
        #poi_menu
        menu_list=[]
        try: 
            for a in range(len(api1_data.get('menus')[0].get('menu_categories'))):
                for b in range(len(api1_data.get('menus')[0].get('menu_categories')[a].get('products'))):
                    menu = api1_data.get('menus')[0].get('menu_categories')[a].get('products')[b].get('name')
